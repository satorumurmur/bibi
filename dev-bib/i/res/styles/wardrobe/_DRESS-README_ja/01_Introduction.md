...
次の構成図は、配布されたままのビビの構成ファイル・フォルダのうち、ドレスの追加・変更に関係するファイル・フォルダだけを抜き出したものです。
...
すると、さきに挙げた構成図にある wardrobe ディレクトリ（dev-bib/i/res/styles/wardrobe/）に、`--dress-template--20191218-153045` といった、生成日時を含む名前のフォルダが生成されます。
これが、これから新しく作成するドレスのテンプレートになるフォルダで、中身にはドレスを構成するソースファイルのすべてが含まれています。
まずは、このフォルダを任意の名称に変えてください。ここでは仮に cocktail という名前にしておきます。

なお、もし既に bib/i/res/styles/wardrobe/ ディレクトリにドレスフォルダを追加してある場合、あらたに作るドレスの名前がそれと重複していると、あとの生成処理で上書きすることになってしまいますので、注意してください。
