...
### "play-with-new-window" …… 貼り付け状態で再生ボタン・目次項目をクリック／タップしたとき、新しいウィンドウで開くかどうか

"" または false にすると、かならず埋め込まれたまま本を開きます。
"always" にすると、かならず新しいウィンドウで開きます。
"handheld" にすると、iOS / Android デバイスの場合だけ新しいウィンドウで開きます。

かつてはいつも "handheld" と同じ挙動でした。
スマートフォンでも埋め込んだまま起動したいという要望はちらほらとあったのですが、読むのに十分なパフォーマンスが出なかったためです。
が、どうやらそろそろまともに動くようになったようなので、任意で選べるようにしました。
ただ、"" または false の場合、狭い画面内のさらに小さなフレームで開くことになるため、読者に不便を強いることがないかよく気をつけてください。
（メニュー内のボタンからいつでも別ウィンドウで開きなおせるとはいえ）
...
値には、パスをそのまま指定します。
クロスオリジンアクセスが禁止されたサーバにある本を別のサーバのビビで表示させるときなど、動かない場合があります。
